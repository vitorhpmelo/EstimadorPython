...
def load_flow_FACTS(graph,prt=0,tol=1e-6,inici=-1,itmax=20):
    ...
    z=z+zPf
    FACTSini(graph,useDFACTS=1)#initialize FACTS value
    Vinici_lf(graph,useDBAR=inici,var_t=var_t,var_x=var_x,z=z) #initialize load flow variables, if -1 uses DC load flow to initialize, if 1 uses DBAR voltage magnitudes, if 0 uses flat start
    dz=np.zeros(len(z))
