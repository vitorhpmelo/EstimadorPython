...
for sys in sistemas:
    ...
    ax[0][i].semilogy(dNormal_7[sys].it,dNormal_7[sys].normgrad/normaliz,label=r"$\sigma_{vir} = 10^{-7}$",ls="--",marker="x",color="m")    
    normaliz=dNormal_8[sys].normgrad[0]
    ax[0][i].semilogy(dNormal_8[sys].it,dNormal_8[sys].normgrad/normaliz,label=r"$\sigma_{vir} = 10^{-8}$",ls="--",marker="*",color="b") 
    normaliz=dNormal_9[sys].normgrad[0]
    ax[0][i].semilogy(dNormal_9[sys].it,dNormal_9[sys].normgrad/normaliz,label=r"$\sigma_{vir} = 10^{-9}$",ls="--",marker="*",color="c") 
    ...
    ax[1][i].semilogy(dQR_7[sys].it,dQR_7[sys].normgrad/normaliz,label=r"$\sigma_{vir} = 10^{-7}$",ls="-.",marker="x",color="m")    
    normaliz=dQR_8[sys].normgrad[0]
    ax[1][i].semilogy(dQR_8[sys].it,dQR_8[sys].normgrad/normaliz,label=r"$\sigma_{vir} = 10^{-8}$",ls=":",marker="*",color="b")    
    normaliz=dQR_9[sys].normgrad[0]
    ax[1][i].semilogy(dQR_9[sys].it,dQR_9[sys].normgrad/normaliz,label=r"$\sigma_{vir} = 10^{-9}$",ls=":",marker="v",color="c")   
    
    ax[1][i].hlines(y=1e-9,xmin=0,xmax=max(dQR_9[sys].it),colors="r",label="crit de conv")   
